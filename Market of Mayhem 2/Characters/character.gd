...
func _physics_process(delta: float) -> void:
	...
	DebugDraw.draw_gizmo(transform, Color(Color.RED))
...
func move_my_ass(delta : float, direction : Vector2):
	if impulse_applied:
		...
		if player_controlled:
			new_direction = (global_transform.basis * Vector3(direction.x, 0, direction.y)).normalized()
		else:
			new_direction = Vector3(direction.x, 0, direction.y)
...
#			await anims.animation_finished
			...
			if get_ammo() <= 0: controller.can_fire = false
			if player_controlled:
...
func set_is_slipping(value) -> void:
	# Hacked together slip mechanic. Could be made better. 
	if not slip_timer.is_stopped():
		return
	is_slipping = value
	if value == true:
		# Wanted to use ragdolling to let limbs fail but couldn't figure it out.
